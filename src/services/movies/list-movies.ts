  ...
  handle: async (request: Request, response: Response) => {
    try {
      ...
      const movies = await getConnection().manager.find(Movies, {
        where: `movies.genre LIKE '%${queryParams.genre}%' OR movies.title LIKE '%${queryParams.title}%' OR
        OR movies.director LIKE '%${queryParams.director}%' OR movies.cast LIKE %${queryParams.casts}%`,
      });
